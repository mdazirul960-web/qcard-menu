...
export default function Home() {
  ...
  const [currentHeroIndex, setCurrentHeroIndex] = useState(0); // Tracks active Hero Slide
  ...
  const adItem = menuData.items.find(i => i.type === 'ad_banner');
  const foodItems = menuData.items.filter(i => i.type !== 'ad_banner');
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans pb-24">
        ...
        <div className="flex items-center justify-between px-4 pt-4 mb-3">
           ...
           <div className="text-right">
              <div className="flex items-center justify-end gap-1 text-gray-900 font-bold text-lg leading-none">
                {menuData.cafe_details.name}
              </div>
              <p className="text-xs font-medium text-gray-500 mt-0.5">{menuData.cafe_details.location}</p>
           </div>
        </div>
        <div className="px-4">
          <div className="relative shadow-sm">
            <input 
              type="text" 
              placeholder="Search for 'Wrap' or 'Coffee'..." 
              value={searchQuery}
      ...
      {/* 2. HERO SECTION (With Active Dots) */}
      <div className="pt-4 pb-2 relative">
        ...
        {/* HERO PAGINATION DOTS (The new logic) */}
          ...
          {heroSlides.map((_, i) => (
            <div 
              ...
              className={`h-1.5 rounded-full shadow-sm transition-all duration-300 ${
                currentHeroIndex === i ? 'w-4 bg-white' : 'w-1.5 bg-white/50'
              }`}
            ></div>
          ))}
        </div>
          ...
          {menuData.categories.map((cat, i) => (
            ...
            >
              <div className={`w-16 h-16 rounded-full p-0.5 border-2 transition-all ${
                activeCategory === cat 
                ? 'border-qcard-purple shadow-md scale-105' 
