  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans pb-24">
      ...
      <div className="flex gap-3 px-4 mb-4 mt-2 overflow-x-auto no-scrollbar">
        {filters.veg?.show && (
          <button onClick={() => setFilterVeg(!filterVeg)} className={`border rounded-lg px-3 py-1.5 text-xs font-bold whitespace-nowrap shadow-sm transition-all ${filterVeg ? 'bg-qcard-purple text-white border-qcard-purple' : 'bg-white text-gray-700 border-gray-300'}`}>{filterVeg ? `✓ ${filters.veg.label}` : filters.veg.label}</button>
        )}
        {filters.price?.show && (
          <button onClick={() => setFilterPrice(!filterPrice)} className={`border rounded-lg px-3 py-1.5 text-xs font-bold whitespace-nowrap shadow-sm transition-all ${filterPrice ? 'bg-qcard-purple text-white border-qcard-purple' : 'bg-white text-gray-700 border-gray-300'}`}>{filterPrice ? `✓ ${filters.price.label}` : filters.price.label}</button>
        )}
        {filters.rating?.show && (
          <button onClick={() => setFilterRating(!filterRating)} className={`border rounded-lg px-3 py-1.5 text-xs font-bold whitespace-nowrap shadow-sm transition-all ${filterRating ? 'bg-qcard-purple text-white border-qcard-purple' : 'bg-white text-gray-700 border-gray-300'}`}>{filterRating ? `✓ ${filters.rating.label}` : filters.rating.label}</button>
        )}
      </div>
